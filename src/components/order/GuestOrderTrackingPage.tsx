...
import { useGuestOrder, useGuestOrderTracking, useOrder, useOrderTracking } from "@/hooks/useApi";
...
const OrderTrackingPage = () => {
  ...
  const {
    ...
    error: orderError,
  } = useGuestOrder(guestToken || "")
  const { data: trackingData } = useGuestOrderTracking(guestToken || "")
      ...
      <div className="container-wide max-w-2xl mx-auto">
        ...
        >
          ...
          <h1 className="text-3xl font-display font-bold text-foreground">
            {order?.orderNumber || `ORD-${guestToken?.slice(-6).toUpperCase()}`}
          </h1>
        </motion.div>
